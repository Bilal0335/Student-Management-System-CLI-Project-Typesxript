...
class Student {
    ...
    payTuition(amount) {
        if (this.balance >= amount) {
            this.balance -= amount;
            console.log(`$ ${chalk.bold.greenBright(amount)} has been paid by ${chalk.bold(this.name)}.`);
    ...
    showStatus() {
        ...
        console.log(chalk.bold.white(`  Age:  ${chalk.bold.magentaBright(this.age)} year`));
        console.log(chalk.bold.white(`  Grade Level:  ${chalk.bold.magentaBright(this.gradeLevel)}`));
...
class StudentManager {
    ...
    addStudent(name, age, gradeLevel) {
        const toTitleCaseName = toTitleCase(name);
        const student = new Student(toTitleCaseName, age, gradeLevel);
        this.students.push(student);
        console.log(chalk.bold("-".repeat(60)));
        console.log(`'${chalk.bold.blueBright(toTitleCaseName)}' added successfully. Student ID: ${chalk.greenBright(student.id)}`);
        console.log(chalk.bold("-".repeat(60)));
    }
    removeStudent(studentId) {
        ...
        if (index !== -1) {
            ...
            console.log(chalk.bold("-".repeat(60)));
            console.log(`'${chalk.bold.blueBright(removedStudent.name)}' with ID ${chalk.greenBright(removedStudent.id)} has been removed.`);
            console.log(chalk.bold("-".repeat(60)));
        }
        else {
    ...
    enrollStudent(studentId, courses) {
        const student = this.findStudent(studentId);
        if (student) {
            courses.forEach((course) => student.enrollCourse(course));
            console.log(chalk.bold("-".repeat(60)));
            console.log(`'${chalk.bold(student.name)}' enrolled in courses: ${chalk.bold.blueBright(courses.join(", "))}`);
            console.log(chalk.bold("-".repeat(60)));
...
async function main() {
    ...
    console.log(chalk.yellowBright.bold("        **BilalCode - Student Management System**          "));
    console.log(chalk.yellowBright.bold("=".repeat(60)));
    ...
    while (true) {
        ...
        switch (choice) {
            case "Add Student":
                ...
                studentManager.addStudent(studentDetails.name, age, gradeLevel.gradeLevel);
                break;
            case "Enroll Student":
                ...
                if (isNaN(studentId)) {
                    console.log(chalk.red("Invalid student ID entered. Please try again."));
                    break;
                }
